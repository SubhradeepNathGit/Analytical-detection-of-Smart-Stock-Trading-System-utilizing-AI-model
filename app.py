...
def train_neural_prophet(ticker_str, start_date_str, end_date_str, _stocks_df):
...
with tab3:
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scattergl(x=stock_data['Date'], y=close_converted, mode='lines', name='Closing Price', line=dict(color='#FFFFFF', width=2)))
...
with tab5:
    ...
    with st.spinner(" AI Model is computing 300-days future trajectory"):
        forecast, actual_prediction = train_neural_prophet(user_input, str(start_date), str(end_date), stocks)
